module.exports = (function() {
  ...
  var handles_cbs = [
    ...
    function(e) {  //  TOP [4]
      dim.y = (dim.y2 - e.y < opts.mch ? dim.y2 - opts.mch : e.y);
    },
    function(e) {  //  RIGHT [5]
      dim.x2 = (e.x - dim.x < opts.mcw ? dim.x + opts.mcw : e.x);
    },
    function(e) {  //  BOTTOM [6]
      dim.y2 = (e.y - dim.y < opts.mch ? dim.y + opts.mch : e.y);
    },
    function(e) {  //  LEFT [7]
      dim.x = (dim.x2 - e.x < opts.mcw ? dim.x2 - opts.mcw : e.x);
    },
  ];
  ...
  function ImageCropper(selector, img_src, tmp_opts) {
    ...
    if (opts.mcw > 80) {
      dim.x2 = dim.w = opts.mcw;
    }
    if (opts.mch > 80) {
      dim.y2 = dim.h = opts.mch;
    }
    if (opts.fs) {
